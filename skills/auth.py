...
def register_shop(
    ...
    try:
        cur = conn.execute("SELECT shop_id FROM shops WHERE LOWER(shop_name) = ?", (name.lower(),))
        if cur.fetchone():
        ...
        with immediate_transaction(conn):
            cur = conn.execute("""
                ...
                VALUES (?, ?, ?, ?)
            """, (name, pwd_hash, shop_address.strip() if shop_address else None, shop_gstin.strip() if shop_gstin else None))
...
def login_shop(telegram_id: str, shop_name: str, password: str) -> Dict[str, Any]:
    ...
    conn = get_db_connection()
    try:
        cur = conn.execute("SELECT * FROM shops WHERE LOWER(shop_name) = ?", (name.lower(),))
        shop = cur.fetchone()
        if not shop:
...
def logout_user_session(telegram_id: str) -> bool:
    ...
    try:
        with immediate_transaction(conn):
            conn.execute("DELETE FROM user_sessions WHERE telegram_id = ?", (str(telegram_id),))
        return True
    finally:
        conn.close()
