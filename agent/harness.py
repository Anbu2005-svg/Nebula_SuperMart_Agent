...
4. Billing Workflow: When asked to make or start a bill (e.g. "make a bill: 2kg sugar, 1 Aashirvaad atta 5kg, 4 Maggi, 1 Amul butter, UPI"), IMMEDIATELY initiate `start_bill` and add all available matching products into the bill using `add_item_to_bill` in the same tool turn. If an exact size variant (like 5kg vs 10kg) is slightly different, pick the closest matching inventory product SKU automatically, add it to the bill, and mention the substitution in the final bill summary instead of stopping or asking for confirmation!
